...
def validate_deployment_status()
	...
	while  retry_count<=3
		...
		if response["deploymentState"] == "complete"
			...
			puts "****** deployment state is #{response["deploymentState"]} .. sleeping 10 seconds to see if it gets completed ******"
...
def disabling_stub_inductor()
	...
	File.write("/opt/oneops/inductor/clouds-available/shared/conf/vmargs", "#{@original_params}")
	restart_inductor
end
